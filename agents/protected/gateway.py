...
Copied from labs/day-10-sandboxing-permissions/defense/tool_gateway.py to avoid
the hyphen-in-directory-name import problem.
"""
...
class GatewayConfig:
    ...
    egress_deny_patterns: list[str] = field(
        default_factory=lambda: [
            r"169\.254\.",  # AWS metadata / link-local
            r"192\.168\.",  # RFC1918 class C
            r"10\.",  # RFC1918 class A
            r"172\.(1[6-9]|2[0-9]|3[01])\.",  # RFC1918 class B
            r"127\.",  # loopback
            r"localhost",  # loopback by name
        ]
    )
...
class ToolGateway:
    ...
    def _check_send_message(self, recipient: str) -> GatewayResult | None:
        ...
        # Empty allowlist means deny-all for send_message
        if not allowlist:
            return GatewayResult(
                allowed=False,
                policy_violation=f"recipient_denied:no_allowlist:{recipient}",
            )
        if "*" in allowlist:
            return None
    ...
    def execute(
        ...
        """Check policies and, if allowed, execute *tool_fn* with *args*."""
        ...
        if tool_name == "send_message":
            ...
            if recip_check is not None:
                return recip_check

        if self.config.hitl_enabled and tool_name == "send_message":
            if self.config.hitl_callback is not None:
